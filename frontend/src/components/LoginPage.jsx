...
function LoginPage() {
    ...
    // Check for Google OAuth ID token in URL fragment on mount
    useEffect(() => {
        const handleGoogleOAuthCallback = async () => {
            const hash = window.location.hash;
            // Google OAuth returns id_token in the hash
            if (hash && hash.includes('id_token=')) {
                ...
                if (idToken) {
                    ...
                    try {
                        ...
                        alert(
                            'Authentication failed. Please check if you are authorized.'
                        );
                    } finally {
                        setLoading(false);
                    }
    ...
    const handleSecuredAccess = async () => {
        ...
        try {
            ...
            const response = await fetch(`${API}/api/auth/google/url`);
    ...
    return (
        <div className='min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-[#1a1d23] dark:to-[#14161a] flex flex-col items-center justify-center p-4 relative overflow-hidden transition-colors duration-500'>
            ...
            <div className='absolute inset-0 overflow-hidden'>
                ...
                <div className='absolute top-20 right-20 text-4xl opacity-10 dark:opacity-5 animate-float hidden sm:block'>
                    ⚔️
                </div>
                <div
                    className='absolute bottom-32 left-32 text-3xl opacity-10 dark:opacity-5 animate-float hidden sm:block'
                    style={{animationDelay: '1s'}}
                >
                    ☠️
                </div>
                <div
                    className='absolute top-40 left-40 text-2xl opacity-10 dark:opacity-5 animate-float hidden md:block'
                        ...
                        <img
                            ...
                            onError={(e) => {
                                ...
                                e.target.parentElement.innerHTML =
                                    '<span class="text-4xl sm:text-5xl font-bold text-slate-900 dark:text-white">P</span>';
                            }}
                        />
                    </div>
                        ...
                        >
                            <div className='flex items-center justify-between'>
                                ...
                                {hoveredButton === 'secure' && (
                                    <span className='text-red-500 animate-pulse hidden sm:inline'>
                                        ⚔️
                                    </span>
                                )}
                            </div>
